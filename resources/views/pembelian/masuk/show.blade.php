...
<style>
    ...
    @media print {
        ...
        #faktur-print, #faktur-print * { visibility: visible; }
        #faktur-print { position: absolute; top: 0; left: 0; width: 100%; padding: 20px; }
                ...
                <div class="table-responsive">
                    <table class="table table-bordered align-middle">
                        ...
                        <tbody>
                            ...
                            @php
                                $exp    = $d->batch?->expired_date;
                                $status = $d->batch?->status_expired ?? 'no_expired';
                            @endphp
                            <tr>
                                <td>{{ $i + 1 }}</td>
                                ...
                                <td>
                                    ...
                                    @elseif($status === 'expired')
                                        ...
                                        <div class="small text-danger">{{ $exp->format('d/m/Y') }}</div>
                                    @elseif($status === 'warning')
                                        <span class="badge bg-warning text-dark">Segera</span>
                                        <div class="small text-warning fw-semibold">{{ $exp->format('d/m/Y') }}</div>
                                    @else
                                        <span class="text-success small">{{ $exp->format('d/m/Y') }}</span>
                                    @endif
                                </td>
    ...
    <div style="font-family:Arial,sans-serif; max-width:800px; margin:auto; padding:40px 50px;">
        ...
        <div style="margin-bottom:20px;">
            <table style="font-size:13px; width:50%;">
                ...
                <tr><td style="padding:3px 0;"><strong>Alamat</strong></td><td>: {{ $pembelian->supplier->alamat }}</td></tr>
                @endif
                @if(!empty($pembelian->supplier->telepon))
                <tr><td style="padding:3px 0;"><strong>Telepon</strong></td><td>: {{ $pembelian->supplier->telepon }}</td></tr>
                @endif
                @if(!empty($pembelian->keterangan))
                <tr><td style="padding:3px 0;"><strong>Keterangan</strong></td><td>: {{ $pembelian->keterangan }}</td></tr>
                @endif
            </table>
        </div>
        ...
        <table style="width:100%; border-collapse:collapse; font-size:13px; margin-bottom:20px;">
            <thead>
                <tr style="background-color:#f0f0f0;">
                    <th style="border:1px solid #ccc; padding:8px; text-align:center; width:30px;">No</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="border:1px solid #ccc; padding:7px; text-align:center;">
                        {{ $d->batch?->expired_date?->format('d/m/Y') ?? '—' }}
                    </td>
